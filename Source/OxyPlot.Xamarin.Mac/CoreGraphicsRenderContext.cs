...
namespace OxyPlot.Xamarin.Mac
{
    ...
    public class CoreGraphicsRenderContext : RenderContextBase, IDisposable
    {
        ...
        {
            if (stroke.IsVisible () && thickness > 0) {
                this.SetAlias (edgeRenderingMode != EdgeRenderingMode.PreferSpeed);
                this.SetStroke (stroke, thickness, dashArray, lineJoin);

                using (var path = new CGPath ()) {
                    var convertedPoints = (edgeRenderingMode != EdgeRenderingMode.PreferSpeed ? points.Select (p => p.ConvertAliased ()) : points.Select (p => p.Convert ())).ToArray ();
